...
import pandas as pd
recommendations_df = pd.read_csv("cf/item_cf_recommendations.csv")

recommendations_df = recommendations_df.sort_values(by="user_id").reset_index(drop=True)

# 保存排序后的推荐结果
recommendations_df.to_csv("cf/item_cf_recommendations_sorted.csv", index=False)

print("推荐结果已按 user_id 排序并保存为 'cf/item_cf_recommendations_sorted.csv'。")


#python cf/cf_item.py